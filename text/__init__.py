  ...
  while len(text):
    ...
    if not m:
      ...
      if dictionary is not None:
        #words = _words_re.findall(clean_text)
        words = clean_text
        clean_text = []
        for (num, word) in enumerate(words.split(' ')):
          if num == 0:
            clean_text.append(get_arpabet('_', dictionary))
          if word[-1] != '.':
            clean_text.append(get_arpabet(word, dictionary))
            clean_text.append(get_arpabet(' ', dictionary))
          else:
            clean_text.append(get_arpabet(word[:-1], dictionary))
            clean_text.append(get_arpabet('.', dictionary))
        #clean_text = [
        #  get_arpabet(word, dictionary)
