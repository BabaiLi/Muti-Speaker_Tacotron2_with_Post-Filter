...
def predict(spectrogram, model_dir=None, speaker=None, params=None, device=torch.device('cuda'), fast_sampling=False):
  # Lazy load model.
  if not model_dir in models:
    if Path(f'{model_dir}/weights.pt').exists():
      checkpoint = torch.load(f'{model_dir}/weights.pt')
    else:
      checkpoint = torch.load(model_dir)
    model = DiffWave(Parameter).to(device)
  ...
  with torch.no_grad():
    ...
    audio = torch.randn_like(spectrogram)
    noise_scale = torch.from_numpy(alpha_cum**0.5).float().unsqueeze(1).to(device)
    for n in range(len(alpha) - 1, -1, -1):
      #        alpha = 1 - beta
      #        sqrt(alpha) * x0 + sqrt(1 - alpha) * z by training
      c1 = 1 / alpha[n]**0.5
      c2 = beta[n] / (1 - alpha_cum[n])**0.5
      audio = c1 * (audio - c2 * model(audio, spectrogram, torch.tensor([T[n]], device=audio.device)).squeeze(1))
      if n > 0:
        noise = torch.randn_like(audio)
        sigma = ((1.0 - alpha_cum[n-1]) / (1.0 - alpha_cum[n]) * beta[n])**0.5 #const
        audio += sigma * noise
